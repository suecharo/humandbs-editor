...
|   GET    /api/comments/research/:humId       |
|   POST   /api/comments/research/:humId       |
|   DELETE /api/comments/research/:humId/:id   |
...
各 research エントリは `comments` フィールド (任意) を持ち、コメントスレッドを保持する:
...
`comments` は `optional` にすることで既存の editor-state.json との後方互換性を維持する。
...
| Method | Path | 説明 |
|---|---|---|
| GET | `/api/comments/research/:humId` | コメント一覧取得 |
| POST | `/api/comments/research/:humId` | コメント追加 |
| DELETE | `/api/comments/research/:humId/:commentId` | コメント削除 |

#### GET /api/comments/research/:humId

```
200: { comments: ResearchComment[] }
```
...
200: { comment: ResearchComment }
400: { error: string }  -- author 空 / text 空
```

`id` (`crypto.randomUUID()`) と `createdAt` はサーバーで生成する。`text` の最大長は 10000 文字。

#### DELETE /api/comments/research/:humId/:commentId

```
200: { deleted: true }
```

指定 `id` のコメントを配列から削除する。存在しない `id` でも 200 を返す (冪等)。

コメントは editing lock と独立しており、readOnly モードでも投稿・閲覧できる。
